class ItemDecorator < Draper::Decorator
  ...
  def add_to_collection
	...
	def featured_image_tag
		if model.comp.present?
			# cl_image_tag(model.images.where(image_type: "Comp Web").first.filemaker_id + ".jpg", :alt => "#{model.filemaker_id} Image", class: "webcomp")
			cl_image_tag "featured_" + model.comp.filemaker_id + ".jpg", :width => :auto, :crop => :limit, :dpr => :auto, :responsive_placeholder => "#{asset_path 'detail_loading.svg'}", alt: "Item: #{model.filemaker_id}", class: "img-responsive cld-responsive"
		else
			image_tag "data:image/svg+xml;base64,"
		end
	end
